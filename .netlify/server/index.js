...
function error_to_pojo(error2, get_stack) {
  if (error2 instanceof import_index2.H) {
    return {
...
async function render_endpoint(event, mod, state) {
  ...
  try {
    ...
    if (error2 instanceof import_index2.R) {
...
async function render_response({
  ...
  let rendered;
  const stack = error2 instanceof import_index2.H ? void 0 : error2 == null ? void 0 : error2.stack;
  if (error2 && options.dev && !(error2 instanceof import_index2.H)) {
    error2.stack = options.get_stack(error2);
  }
  if (page_config.ssr) {
    const props = {
      stores: {
        page: (0, import_index3.w)(null),
        navigating: (0, import_index3.w)(null),
        updated
      },
    ...
    if (leaf_node.server && event.request.method !== "GET" && event.request.method !== "HEAD") {
      try {
        ...
        if (handler) {
          ...
          mutation_error = (0, import_index2.e)(405, "Method not allowed");
        }
      } catch (e) {
        if (e instanceof import_index2.R) {
    ...
    for (let i = 0; i < nodes.length; i += 1) {
      ...
      if (node) {
        try {
          ...
          const status2 = error2 instanceof import_index2.H ? error2.status : 500;
          while (i--) {
            if (page.errors[i]) {
...
async function handle_json_request(event, options, mod) {
  ...
  try {
    const result = await handler.call(null, event);
    if (result == null ? void 0 : result.errors) {
      return (0, import_index2.j)({ errors: result.errors }, { status: result.status || 400 });
    }
    return new Response(void 0, {
      status: 204,
    ...
    return (0, import_index2.j)(error_to_pojo(error2, options.get_stack), {
      status: error2 instanceof import_index2.H ? error2.status : 500
    });
  }
}
